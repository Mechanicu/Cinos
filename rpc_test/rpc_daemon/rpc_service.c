...
cptr_t rpc_service_register(const unsigned long service_id, const cptr_t server_id)
{
    ...
    if ((service_check = linkhash_get(service_id, service_cap_hash)) != 0) {
        LOG_WARING("RPC service %lu already exists, service_cap:%lu.\n", server_id, service_check);
    ...
    LOG_DEBUG("Register new server id:%lu, cptr:%lx.\n", service_id, service_cap);
    return service_cap;
}
...
cptr_t rpc_client_security_check(const unsigned long service_id)
{
    ...
    LOG_DEBUG("client thread id:%lx, srv_cptr:%lx, srv_id:%lu\n", cur_tid, service_cap, service_id);
    return service_cap;
}
