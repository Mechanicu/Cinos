...
static inline cptr_t mk_rpc_service_cap(const unsigned long server_id)
{
    ...
    if (service_cap != NULL) {
        INIT_LIST_HEAD(&(service_cap->client_list));
...
cptr_t rpc_service_register(const unsigned long service_id, const cptr_t server_id)
{
    ...
    if ((service_check = linkhash_get(service_id, service_cap_hash)) != 0) {
        LOG_WARING("RPC service%lu already exists, server id:%lu.", server_id, service_check);
        return 0;
    }
...
void rpc_service_unregister(const unsigned long service_id)
{
    ...
    LOG_WARING("RPC service%lu not found in hash table.\n", service_id);
}
...
cptr_t rpc_client_security_check(const unsigned long service_id)
{
    ...
    LOG_DEBUG("client thread id:%lu, srv_cptr:%lx, srv_id:%lu\n", cur_tid, service_cap, service_id);
    return service_cap;
}
...
{
    pthread_t cur_tid = pthread_self();
    LOG_DEBUG("client request service, thread id:%lu, srv_cptr:%lx\n", cur_tid, service_cap);
